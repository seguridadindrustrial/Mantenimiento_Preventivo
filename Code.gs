...
function doPost(e) {
    ...
    if (data.tipo === "asignar_averia") {
        return asignarAveria(data.numero, data.tecnicoNombre, data.tecnicoWhatsapp, data.tecnicoCorreo, data.correoReportero);
    }
    ...
    if (data.tipo === "solicitar_cisterna") {
        return solicitarCisterna(data);
    }

    if (data.tipo === "semanario_ruices") {
        return guardarSemanarioRuices(data);
    }
...
function solicitarCisterna(data) {
    var sede = data.sede || data.sedes || "";
    var html = "<h3 style='color:#1976d2;'>Solicitud de Cisterna</h3>" +
        "<b>Sede:</b> " + (sede || "") + "<br>" +
    ...
    try {
        MailApp.sendEmail({
            to: "loly.garcia@caracascatering.com, alberto.blanco@caracascatering.com",
            subject: "Solicitud de Cisterna - " + (sede || ""),
            body: "Solicitud de cisterna\nSede: " + (sede || "") + "\nFecha: " + (data.fecha || "") + "\nHora: " + (data.hora || "") + "\nTecnico: " + (data.tecnico || ""),
            htmlBody: html
        });
        mailEnviado = true;
    } catch (mailErr) {
    ...
    if (!mailEnviado) {
        registrarError("cisterna", (data.fecha || "") + " " + (data.hora || ""), mailError);
    }

    return jsonAveria({ status: "ok", mailEnviado: mailEnviado });
}
...
function guardarSemanarioRuices(data) {
    ...
    for (var p = 0; p < titulos.length; p++) {
        headers.push(titulos[p].titulo || "");
        var respuestas = titulos[p].respuestas || {};
        for (var k in respuestas) {
            if (respuestas.hasOwnProperty(k)) headers.push(k);
        }
        headers.push("");
    }
    headers.push("Descripcion");
    ...
    for (var c = 0; c < existingHeaders.length; c++) {
        var h = String(existingHeaders[c] || "");
        if (h === "Fecha") { row.push(data.fecha || ""); continue; }
        if (h === "Hora") { row.push(data.hora || ""); continue; }
        if (h === "Turno") { row.push(data.turno || ""); continue; }
        if (h === "Sede") { row.push(data.sedes || ""); continue; }
        if (h === "Zona") { row.push(data.zona || ""); continue; }
        if (h === "Tecnico") { row.push(data.tecnico || ""); continue; }
        if (h === "Descripcion") { row.push(data.descripcion || ""); continue; }

        var esTitulo = false;
        for (var tp = 0; tp < titulos.length; tp++) {
            if (String(titulos[tp].titulo || "") === h) {
                currentTitulo = h;
                esTitulo = true;
                break;
            }
        }
        if (esTitulo) { row.push(""); continue; }

        if (h) {
            var val = "";
            for (var p2 = 0; p2 < titulos.length; p2++) {
                if (String(titulos[p2].titulo || "") === currentTitulo &&
                    titulos[p2].respuestas && titulos[p2].respuestas.hasOwnProperty(h)) {
                    val = titulos[p2].respuestas[h];
                    break;
                }
            }
...
function procesarAveria(data) {
    ...
    if (correoJefe) {
        var htmlJefe = "<h3 style='color:#d32f2f;'>Averia " + numero + " reportada por " + (data.empleado || data.tecnico) + "</h3>" +
        ...
        try {
            MailApp.sendEmail({
                to: correoJefe,
                subject: "Averia " + numero + " - " + (data.empleado || data.tecnico) + " - " + (data.equipo || ""),
                body: "Averia " + numero + "\nReporto: " + (data.empleado || data.tecnico) + "\nEquipo: " + (data.equipo || ""),
                htmlBody: htmlJefe
            });
        } catch (eJefe) {}
    }
    ...
    if (correoReportero) {
        ...
        if (realizado === "Si") {
            ...
            htmlReportero = "<h3 style='color:" + colorEstado + ";'>" + emoji + " Averia " + numero + "</h3>" +
                ...
                "<p style='color:#666;'>Si tienes dudas, contacta al departamento de Seguridad Industrial.</p>";
...
function asignarAveria(numero, tecnicoNombre, tecnicoWhatsapp, tecnicoCorreo, correoReportero) {
    var ss = SpreadsheetApp.getActiveSpreadsheet();
    var sheet = ss.getSheetByName("averias");
    if (!sheet) {
        return jsonAveria({ status: "not_found" });
    ...
    var reporteroEsTecnico = false;
    if (empleadoNombre) {
        try {
            var tecSheet = ss.getSheetByName("tecnicos");
            if (tecSheet) {
                var tData = tecSheet.getDataRange().getValues();
                for (var t = 1; t < tData.length; t++) {
                    if (String(tData[t][0] || "").trim() === empleadoNombre) {
                        reporteroEsTecnico = true;
                        break;
                    }
                }
            }
    ...
    var html = "<h3 style='color:#2e7d32;'>Averia Asignada - " + numero + "</h3>" +
        "<b>Hola " + tecnicoNombre + ", se te ha asignado la siguiente averia:</b><br><br>" +
        ...
        (reporteroEsTecnico ? "<b>Reportada por:</b> " + empleadoNombre + "<br>" : "") +
    ...
    if (tecnicoWhatsapp) {
        var mensaje = "Hola " + tecnicoNombre + ", se te ha asignado la averia " + numero + ".\n" +
